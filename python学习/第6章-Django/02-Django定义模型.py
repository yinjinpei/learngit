...
字段选项
    ...
    6，db_index：若值为 True, 则在表中会为此字段创建索引
    7，default：默认值
...
元选项
    1，在模型类中定义类Meta，用于设置元信息
    2，元信息db_table：定义数据表名称，推荐使用小写字母，数据表的默认名称
        <app_name>_<model_name>
    3，ordering：对象的默认排序字段，获取对象的列表时使用，接收属性构成的列表
        class BookInfo(models.Model):
            ...
            class Meta():
                ordering = ['id']
    4，字符串前加-表示倒序，不加-表示正序
        class BookInfo(models.Model):
            ...
            class Meta():
                ordering = ['-id']
